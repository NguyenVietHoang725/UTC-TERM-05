...
public class TrieST<Value> {
    private final int R;        // radix from Alphabet
    private final Alphabet alphabet; // the alphabet
    ...
    private static class Node {
        private Object val;
        private Node[] next; // initialized dynamically
    }

   /**
     * Initializes an empty string symbol table with a given alphabet.
     */
    ...
    /**
     * Default constructor using extended ASCII.
     */
    public TrieST() {
    ...
    private Node put(Node x, String key, Value val, int d) {
        if (x == null) {
            x = new Node();
            x.next = new Node[R]; // Initialize array based on R
        }
        if (d == key.length()) {
            if (x.val == null) n++;
            x.val = val;
    ...
    private void collect(Node x, StringBuilder prefix, Queue<String> results) {
        ...
        for (int c = 0; c < R; c++) {
            prefix.append(alphabet.toChar(c));
    ...
    private void collect(Node x, StringBuilder prefix, String pattern, Queue<String> results) {
        ...
        if (c == '.') {
            for (int ch = 0; ch < R; ch++) {
                prefix.append(alphabet.toChar(ch));
